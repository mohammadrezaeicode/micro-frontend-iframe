...
import {  checkOriginForChild,
  childToParent,
  customEventChildToParent,
  setCustomEvent,
  setEvent, } from "./massagechannle";
  ...
  useEffect(()=>{
    ...
    setTimeout(()=>{
      ...
      if (typeof window !== "undefined") {
        ...
        childToParent('from next');
        if (process.env.NODE_ENV==='production'&&checkOriginForChild()) {
          setCustomEvent((event) => {
            if (typeof event == 'object' && event && 'data' in event) {
              console.log('in next - custom', event.data);
            }
          });
          customEventChildToParent('from next custom');
        }
      }
    })
  })
